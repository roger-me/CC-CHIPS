...
# COLORS — Cyber theme
# ═══════════════════════════════════════════════════════════════════
# Left chip:   bg #132831, text #64CCC2
# Middle chip: bg #FFF700, text #000000
# Right chip:  bg #46151F, text #F7504A
...
# CHIP 1: Left pill (#132831 bg, #64CCC2 text) — folder + path
...
# CHIP: Middle pill (#FFF700 bg, #000000 text) — git info
if [ -n "$git_branch" ]; then
    printf " "
    printf "${FG_MID}${CAP_LEFT}${RESET}"
    printf "${BG_MID}${BOLD}${FG_MID_TEXT} ${ICON_GITHUB} ${ICON_BRANCH} %s${FG_YELLOW}%s${FG_MID_TEXT} ${RESET}" "$git_branch" "$git_dirty"
    printf "${FG_MID}${CAP_RIGHT}${RESET}"
fi

printf " "

# CHIP 3: Right pill (#46151F bg, #F7504A text) — brain + model + monitor + context + dollar + cost
